...
export COMPOSE_PROJECT_NAME=prometheus-metrics
...
application: ${APP_DIR} setup_application ${APP_DIR}/docker-compose.yaml
...
setup_application:
	...
	$(MAKE) apply_dist
	(cd ${APP_DIR} && ${COMPOSER} require --no-progress acseo/${PLUGIN_NAME}="*@dev")
...
test.all: test.composer test.phpunit test.schema ## Run all tests in once
...
platform: up ## Setup the platform tools
.PHONY: platform
